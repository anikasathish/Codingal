...
hands = Hands.Hands(min_detection_confidence=0.7,
...
try:
    dev = AudioUtilities.GetDefaultOutputDevice() if
    hasattr(AudioUtilities, "GetDefaultOutputDevice") else
    AudioUtilities.GetSpeakers()
        volctl =
    dev. EndpointVolume. QueryInterface(IAudioEndpointVolume)
    ...
    WIN = "Hand Gesture Control"; cv2.namedWindow(WIN,
    CV2.WINDOW NORMAL)
...
if res.multi_hand_landmarks and res.multi_handedness:
    for i, hand in enumerate(res.multi_hand_landmarks):
        label =
        res.multi_handedness[i].classification[0].label
        draw.draw_landmarks(img, hand,
    Hands. HAND_CONNECTIONS)
    1m = hand.landmark
    tp=(int(1m[TH].x*w), int(1m[TH].y*h)); ip =
    (int(1m[IX].x*w), int(1m[IX].y*h))
...
except Exception as e: print(f"Volume error:

bar = int(np.interp(dist, [30,300],
[400,150])); pct = int(np.interp(dist, [30,300], [0,100]))
cv2.rectangle(img, (50,150), (85,400),
(255,0,0), 2); cv2.rectangle(img, (50,bar), (85,400),
(255,0,0), cv2.FILLED)
cv2.putText(img, f"{pct}%", (40,450),
...
bar = int(np.interp(dist, [30,300], [0, 100]))
error: {e}")

try: sbc.set_brightness(b)
except Exception as e: print(f"Brightness")

bar = int(np.interp(dist, [30,300],
[400,150])); x1, x2 = W-85, w-50
cv2.rectangle(img, (x1,150), (x2,400),
(0,255,0), 2); cv2.rectangle(img, (x1,bar), (x2,400),
(0,255,0), cv2.FILLED)
cv2.putText(img, f"{b}%", (w-110,450),
