...
if res.multi_hand_landmarks and res.multi_handedness:
    ...
    1m = hand. landmark
tp=(int(1m[TH].x*w), int(1m[TH].y*h)); ip=(int(1m[IX].x*w),
int(1m[IX].y*h))
cv2.circle(img, tp, 10, (255,0,0), cv2.FILLED); cv2.circle(img,
ip, 10, (255,0,0), cv2.FILLED)
...
bar = int(np.interp(dist, [30,300], [400,150])); pct =
int(np.interp(dist, [30,300], [0,100]))
...
bar = int(np.interp(dist, [30,300], [400,150])); x1, x2 =
