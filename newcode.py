...
hands = H.Hands(min_detection_confidence=0.7,
min_tracking_confidence=0.7)
draw = mp.solutions.drawing_utils
pairs = {"middle":("SEPIA","NEGATIVE"), "ring":("BLUR","GLITCH"),
"pinky": ("EDGE", "CARTOON")}
...
def apply(img, t):
	...
	if t == "CARTOON":
			...
			e = cv2.adaptiveThreshold(cv2.medianBlur(g, 7), 255,
...
while True:
	...
	if res.multi_hand_landmarks:
		...
		lm = hand.landmark; tips = {k: (int(lm[v].x*w),
		int(lm[v].y*h)) for k,v in ids.items()}
	...
	if not pinch:
		t = next((k for k in pairs if abs(tx-tips[k][0]) < TT
		and abs(ty-tips[k][1]) < TT), None)
	...
	if capture:
		name = f"picture_{int(now)}.jpg"; cv2.imwrite(name, out)
		print("Saved:", name)
